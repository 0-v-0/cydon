...
function zencode(input, fabbr) {
	...
	for (i = 0, len = s.length; i < len; i++) {
		let set = s[i], tag = "", content = "", cls = [], attr = [""],
			tags = [], lasttag, prevg;
		switch (set) {
			case "^":
					...
					for (l = tags.length; n < l; n++) {
						...
						switch (buffer[0]) {
							...
							case "#":
								...
								break;
							case "[":
								attr.push(buffer.slice(1, -1).replace(/([^=\s]+)(=?)('[^']*'|"[^"]*"|[^"'\s]*)/g,
...
export default (input, indent, f) => {
	if (indent)
		input = extractTabs(input, indent);
	return zencode(input, f || fabbr);
}